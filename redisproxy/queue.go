...
func NewQueue(proxy *redisProxy, name string) *Queue {
	s := new(Queue)
	s.Name = name
	s.proxy = proxy
	return s
...
func newQueueProducer(proxy *redisProxy, topic string) *queueProducer {
...
func newQueueConsumer(proxy *redisProxy, topics []string) *queueConsumer {
	s := new(queueConsumer)
